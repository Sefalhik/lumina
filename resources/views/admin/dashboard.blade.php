...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <div class="border border-primary/20 bg-base-200 p-6 hover:border-primary/60 transition-all duration-300">
        <p class="text-[10px] tracking-[0.3em] uppercase text-base-content/40 mb-2">{{ __('admin.card_status_label') }}</p>
        <p class="font-display text-sm text-primary">{{ __('admin.card_status_value') }}</p>
    </div>
    <div class="border border-primary/20 bg-base-200 p-6 hover:border-primary/60 transition-all duration-300">
        <p class="text-[10px] tracking-[0.3em] uppercase text-base-content/40 mb-2">{{ __('admin.card_operator_label') }}</p>
        <p class="font-display text-sm text-primary">⬡ {{ auth()->user()->name }}</p>
    </div>
    <div class="border border-primary/20 bg-base-200 p-6 hover:border-primary/60 transition-all duration-300">
        <p class="text-[10px] tracking-[0.3em] uppercase text-base-content/40 mb-2">{{ __('admin.card_security_label') }}</p>
